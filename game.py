...
class Game(State):
    def __init__(self, painful=False):
        ...
        player.SPRITE = pygame.image.load(
            "assets/images/player.png").convert_alpha()
        player.STUNNED_SPRITE = pygame.image.load(
            "assets/images/player_stunned.png").convert_alpha()
        player.SURPRISED_SPRITE = pygame.image.load(
            "assets/images/player_surprised.png").convert_alpha()
        enemy.STUNNED_SPRITE = pygame.image.load(
            "assets/images/enemy_stunned.png").convert_alpha()
        enemy.SURPRISED_SPRITE = pygame.image.load(
            "assets/images/enemy_surprised.png").convert_alpha()
        enemy.SPRITE = pygame.image.load(
            "assets/images/enemy.png").convert_alpha()
        enemy.KEY_TAKEN_SPRITE = pygame.image.load(
            "assets/images/key_taken.png").convert_alpha()
        hud.HEART_SPRITE = pygame.image.load(
            "assets/images/heart.png").convert_alpha()
        hud.HEART_EMPTY_SPRITE = pygame.image.load(
            "assets/images/heart_empty.png").convert_alpha()
        hud.STAMINA_SPRITE = pygame.image.load(
            "assets/images/stamina_full.png").convert_alpha()
        hud.STAMINA_EMPTY_SPRITE = pygame.image.load(
            "assets/images/stamina_empty.png").convert_alpha()
        hud.HEART_WEAK_SPRITE = pygame.image.load(
            "assets/images/weak_heart.png").convert_alpha()
        hud.KEY_SPRITE = pygame.image.load(
            "assets/images/key.png").convert_alpha()  # TODO another image
        pickupable.LITTLE_HEART_SPRITE = pygame.image.load(
            "assets/images/little_heart.png").convert_alpha()
        pickupable.LITTLE_HEART_WEAK_SPRITE = pygame.image.load(
            "assets/images/little_weak_heart.png").convert_alpha()
        pickupable.KEY_SPRITE = pygame.image.load(
            "assets/images/key.png").convert_alpha()
        sword.SPRITE = pygame.image.load(
            "assets/images/sword.png").convert_alpha()
        sword.SWANG_SPRITE = pygame.image.load(
            "assets/images/sword_swang.png").convert_alpha()
        obstacle.BOX_SPRITE = pygame.image.load(
            "assets/images/box.png").convert_alpha()

        # SOUND INITIALIZATION
        sword.SWING_SOUNDS = [
            pygame.mixer.Sound('assets/sounds/sword_{}.wav'.format(i + 1)) for
            i in range(5)]
        sword.CLING_SOUND = pygame.mixer.Sound(
            'assets/sounds/sword_hit_reject.wav')
        ...
        player.DEATH_SOUND = pygame.mixer.Sound(
            'assets/sounds/hero_death_extra_details.wav')
        player.HEAL_SOUND = pygame.mixer.Sound(
            'assets/sounds/focus_health_heal.wav')
        player.HEARTBEAT_SOUND = pygame.mixer.Sound(
            'assets/sounds/heartbeat_B_01.wav')
        player.STEPS_SOUND = pygame.mixer.Sound(
            'assets/sounds/hero_run_footsteps_stone.wav')
        player.STEPS_SOUND.set_volume(1.5)  # TODO tune
        player.PICKUP_SOUND = pygame.mixer.Sound(
            'assets/sounds/shiny_item_pickup.wav')

        enemy.DASH_SOUND = pygame.mixer.Sound(
            'assets/sounds/ruin_fat_sentry_sword.wav')
        enemy.DASH_SOUND.set_volume(0.5)  # TODO tune
        enemy.DASH_STATS["sound"] = enemy.DASH_SOUND
        enemy.STARTLE_SOUNDS = [pygame.mixer.Sound(
            'assets/sounds/Ruins_Sentry_Fat_startle_0{}.wav'.format(i + 1)) for
            i
            in
            range(2)]
        enemy.ATTACK_SOUNDS = [pygame.mixer.Sound(
            'assets/sounds/Ruins_Sentry_Fat_attack_0{}.wav'.format(i + 1)) for
            i
            in
            range(3)]
        enemy.DEATH_SOUNDS = [pygame.mixer.Sound(
            'assets/sounds/Ruins_Sentry_death_0{}.wav'.format(i + 1)) for i in
            range(3)]
        enemy.HEAL_SOUND = pygame.mixer.Sound(
            'assets/sounds/focus_health_heal.wav')  # TODO find another sound

        enemy.HIT_SOUND = pygame.mixer.Sound('assets/sounds/enemy_damage.wav')

        obstacle.BOX_BREAK_SOUNDS = [pygame.mixer.Sound(
            'assets/sounds/breakable_wall_hit_{}.wav'.format(i + 1)) for i
            in
            range(2)]

        self.WIN_SOUND = pygame.mixer.Sound(
            'assets/sounds/secret_discovered_temp.wav')
        ...
        self.wall_group = base.AdvancedGroup(self.render_group,
                                             ...
                                             obstacle.Wall(
                                                 pygame.Rect(1400, 900, 200,
                                                             500)),
                                             obstacle.Wall(
                                                 pygame.Rect(1400, 1600, 200,
                                                             300)),

                                             # horizontal center walls
                                             obstacle.Wall(
                                                 pygame.Rect(100, 900, 500,
                                                             200)),
                                             obstacle.Wall(
                                                 pygame.Rect(900, 900, 500,
                                                             200)),
                                             obstacle.Wall(
                                                 pygame.Rect(1600, 900, 500,
                                                             200)),
                                             obstacle.Wall(
                                                 pygame.Rect(2400, 900, 500,
                                                             200)),
                                             # pillars
                                             obstacle.Wall(
                                                 pygame.Rect(900, 400, 200,
                                                             200), 90),
                                             obstacle.Wall(
                                                 pygame.Rect(1400, 400, 200,
                                                             200), 120),
                                             obstacle.Wall(
                                                 pygame.Rect(1900, 400, 200,
                                                             200), 150),
                                             obstacle.Wall(
                                                 pygame.Rect(1800, 1300, 200,
                                                             200), 30),

                                             # border walls
                                             obstacle.Wall(
                                                 pygame.Rect(100, 0, 2800,
                                                             100)),
                                             obstacle.Wall(
                                                 pygame.Rect(0, 0, 100, 2000)),
                                             obstacle.Wall(
                                                 pygame.Rect(0, 1900, 3000,
                                                             150)),
                                             obstacle.Wall(
                                                 pygame.Rect(2900, 0, 100,
                                                             2000)),
                                             )
    ...
    def distribute_keys(self):
        if len(self.enemy_group) < self.max_keys:
            raise BaseException(
                "um there is not enough enemies to distribute so much keys")
    ...
    def win(self):
        ...
        for i in range(15):
            ...
            self.particle_group.add(
                particle.Blood(self.player.pos + const.V_ZERO,
                               direction * 5,
    ...
    def fade_out(self, action_after_faded):
        self.fade = particle.Fade(const.GAME_FADE_OUT, True,
                                  action_after_faded)
        self.render_group.add(self.fade)
    ...
    def deploy_logo(self):
        self.particle_group.add(
            particle.Title(pygame.image.load(
                'assets/images/{}_level{}.png'.format(
                    1, self.painful * '_painful')),
    ...
    def update(self):
        ...
        for event in pygame.event.get():
            ...
            # and rewrite Controller to listen to events from queue
